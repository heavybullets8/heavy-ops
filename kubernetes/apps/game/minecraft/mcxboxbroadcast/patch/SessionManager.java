...
public class SessionManager extends SessionManagerCore {
        ...
        try {
            ...
            for (String xuid : activeXuids) {
                if (!nonces.containsKey(xuid)) {
                    ...
                    // heavy-ops: a good join lands a transfer ~6s after the nonce. If
                    // nothing lands, NetherNet has stopped taking joins even though every
                    // socket still looks healthy. Watch it and rebuild the session.
                    watchJoin(xuid);
    ...
    /** heavy-ops: epoch millis of the last join-triggered rebuild. */
    private volatile long lastJoinRebuild = 0L;

    /**
     * heavy-ops: watch a join we just minted a nonce for.
     *
     * The transfer path stamps playerHistory().lastSeen(xuid), so if lastSeen is still
     * older than the nonce once the grace period is up, the player never made it
     * through NetherNet and the session needs rebuilding. Rate limited so a handful of
     * failed joins can't turn into a rebuild storm against Xbox LIVE.
     *
     * @param xuid The XUID we just minted a nonce for
     */
    private void watchJoin(String xuid) {
        Instant mintedAt = Instant.now().truncatedTo(ChronoUnit.SECONDS);
        scheduledThreadPool.schedule(() -> {
            try {
                Instant lastSeen = storageManager().playerHistory().lastSeen(xuid);
                if (lastSeen != null && !lastSeen.isBefore(mintedAt)) {
                    return; // transfer landed, the join worked
                }

                long now = System.currentTimeMillis();
                if (now - lastJoinRebuild < 180_000L) {
                    logger.warn("Join for XUID " + xuid + " never completed, rebuild still cooling down");
                    return;
                }
                lastJoinRebuild = now;

                forceRecreateSession("join for XUID " + xuid + " never completed");
            } catch (Exception e) {
                logger.debug("Join watchdog failed for XUID " + xuid + ": " + e);
            }
        }, 60, TimeUnit.SECONDS);
    }
